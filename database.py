...
# same fields as in the model class (model.py)
def create_table():
...
def insert_todo(todo: Todo):
    ...
    count = c.fetchone()[0]
    # start at position 0 if "empty"
    todo.position = count if count else 0
    # paramater substitution syntax 
    # - the :fields are replaced with the values of the corresponding key in dictionary
    with conn:
...
def delete_todo(position):
    ...
    #note: conn.commit() called when execiting the context manager
    with conn:
        c.execute("DELETE from todos WHERE position=:position", {"position": position})
        # shift remaining columns one position down
